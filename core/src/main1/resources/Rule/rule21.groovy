...
status {
    ...
    lastFailCode is "上一次时间停机条件"
    lastDevStatus3 is "上一次用户给定状态"
    lastDevStatus4 is "上一次用户给定状态3"
    lastTime is "上一次时间戳"
}
...
def maintainingStopCode = [1001D, 1002D, 1003D, 1004D, 1011D, 1012D, 1013D, 1014D, 3001D, 3002D, 3003D, 3100D, 15004D,
                           ...
                           2231D, 18002D, 18003D]
def remoteStopCode = [3004D, 18001D]
...
def rule2 = { statusList, condition ->
    switch (condition) {
        case 0: remove(statusList, [3, 4, 5, 6, 7, 9])
            break
        case 2: remove(statusList, [1, 2, 3, 5, 6, 9, 10, 11])
            break
...
def rule4 = { statusList, condition ->
    switch (condition) {
        case 0: remove(statusList, [5, 6, 8])
            break
        case 1: remove(statusList, [1, 2, 5, 10, 11])
            break
        case 2: remove(statusList, [1, 2, 6, 8, 10, 11])
            break
        default: remove(statusList, [1, 2, 5, 6, 8, 10, 11])
    }
    statusList
}
...
def rule6 = { statusList, condition ->
...
task {
    ...
    def time_items = [devStatus, stateAvaile, failcode, devStatus3, devStatus4, yggl, fengsu, fengxang, cangwWd, yeYali]
    //获取timestamp
    def getTime = { items ->
    ...
    if (devStatus4 == null) devStatus4 = lastDevStatus4 else lastDevStatus4 = devStatus4

    def filters = [[devStatus, rule1], [stateAvaile, rule2], [failcode, rule3], [devStatus3, rule4], [devStatus4, rule5],
                   [yggl, rule6]]
