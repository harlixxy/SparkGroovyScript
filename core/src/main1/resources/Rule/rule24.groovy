...
def rule4 = { statusList, condition ->
    switch (condition) {
        case 0: remove(statusList, [5, 6, 8])
            break
        case 1: remove(statusList, [1, 2, 5, 7, 10, 11])
            break
        case 2: remove(statusList, [1, 2, 6, 7, 8, 10, 11])
            break
        default: remove(statusList, [1, 2, 5, 6, 7, 8, 10, 11])
    }
    statusList
}
...
task {
    ...
    def time_items = [devStatus, failcode, stateAvaile, devStatus3, yggl, fengsu, fengxang, cangwWd, yeYali]
    //获取timestamp
    def getTime = { items ->
    ...
    def filters = [[devStatus, rule1], [failcode, rule2], [stateAvaile, rule3], [devStatus3, rule4], [yggl, rule6]]
