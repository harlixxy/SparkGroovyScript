...
task {
    ...
    if (lastStatus != null && lastStatus.value == virtualStatus && virtualStatus == Status.GOOD_RUNNING.code) {
        ...
        first200Timestamp = (virtualStatus == Status.GOOD_RUNNING.code) ? timestamp : null
    }
    ...
    if (lastStatus != null && lastStatus.value == virtualStatus && virtualStatus == Status.ALARM_RUNNING.code) {
        ...
        first400Timestamp = (virtualStatus == Status.GOOD_RUNNING.code) ? timestamp : null
    }

    //最后考虑正常运行或报警运行
    def isOut200or400 = false
    if (lastStatus != null && ((int)lastStatus.value) in [Status.ALARM_RUNNING.code, Status.GOOD_RUNNING.code]
            && ((int)virtualStatus) in [Status.ALARM_RUNNING.code, Status.GOOD_RUNNING.code]) {
        isOut200or400 = isNeedOut(first200or400Timestamp)
    } else {
        first200or400Timestamp = (((int)virtualStatus) in [Status.ALARM_RUNNING.code, Status.GOOD_RUNNING.code]) ? timestamp : null
    }
    ...
    if (yggl != null) {
        ...
        if (clxYouWd != null && yggl.timestamp == clxYouWd.timestamp) {
            if (isOut200) {
                ...
                outClxYouWdFor200 = clxYouWd.value
            }
            if (isOut400) {
                outYgglCangwWdFor400 = yggl.value
                outClxYouWdFor400 = clxYouWd.value
            }
            if (isOut200or400) {
                outYgglCangwWd = yggl.value
