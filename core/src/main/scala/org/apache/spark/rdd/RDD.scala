...
abstract class RDD[T: ClassTag](
  ...
  /**
    * 读取groovy 算法脚本，然后返回装载脚本计算后的数据的RDD
    */

  def groovyCompute(scriptTexts: String,enumStatus:java.util.Map[String,Object],requireNames:java.util.HashMap[String,Object]):RDD[String] = withScope {
   ...
   */
  def countByValue()(implicit ord: Ordering[T] = null):scala.collection.Map[T, Long] = withScope {
  ...
  def countByValueApprox(timeout: Long, confidence: Double = 0.95)
      (implicit ord: Ordering[T] = null)
      : PartialResult[scala.collection.Map[T, BoundedDouble]] = withScope {
    require(0.0 <= confidence && confidence <= 1.0, s"confidence ($confidence) must be in [0,1]")
