...
def rule3 = { statusList, condition1, condition2, range ->
    def result
    if (condition1 || !condition2) result = statusList.findAll { range.contains(it.key) }
    return result == null || result.isEmpty() ? statusList : result
}
...
task {
    ...
    mp = rule2(mp, running != null && running.value == 1D, alarm != null && alarm.value == 1D, [2], [1])
    mp = rule3(mp, commErr != null && commErr.value == 1D,
            commErr_metric.any { entry -> entry != null && entry.value != 0 }, [3, 4])
    mp = rule2(mp, start != null && start.value == 1D, alarm != null && alarm.value == 1D, [7], [6])

    mp = rule2(mp, stop != null && stop.value == 1D, mStop != null && mStop.value == 1D, [8], [10, 22, 21])
    mp = rule2(mp, stop != null && stop.value == 1D, fail != null && fail.value == 1D, [10], [22, 21])
    ...
    def time_items = [fengsu , fengxang , cangwWd , yeYali, stop , running , start , commErr , mStop , fail , alarm ]
    def getTime = { items ->
